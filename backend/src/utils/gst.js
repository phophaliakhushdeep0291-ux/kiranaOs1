...
// GST state codes (the first two digits of a GSTIN). A tax invoice names the
// place of supply by state, not only by code (CGST Rules, rule 46).
...
export function gstStateLabel(code) {
  ...
  if (!/^\d{2}$/.test(normalized) || normalized === "00") return "";
  return GST_STATE_NAMES[normalized] ? `${normalized} - ${GST_STATE_NAMES[normalized]}` : normalized;
}
