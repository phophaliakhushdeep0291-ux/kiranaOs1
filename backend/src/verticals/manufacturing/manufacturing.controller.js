...
export async function returnTradeOrder(req, res, next) { try { res.status(201).json({ success: true, data: await trade.returnTradeOrder(req.shopId, req.params.id, req.body, { userId: req.user?.userId ?? null, deviceId: req.headers?.["x-device-id"] ? String(req.headers["x-device-id"]) : null, req }) }); } catch (e) { next(e); } }
