...
export async function applyStockCount(shopId, locationId, sessionId, actor = {}) {
  ...
  return shape(await db.$transaction(async (tx) => {
    ...
    const productById = new Map(products.map((product, index) => [productIds[index], product]));
    for (const line of session.lines) {
      ...
      console.error(`  LINE ${line.productName} (counted ${line.countedBaseQty}) -> RESOLVED PRODUCT ${product.name}`);
