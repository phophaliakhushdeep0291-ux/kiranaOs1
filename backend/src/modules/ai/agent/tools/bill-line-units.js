...
const canonical = value => aliases[text(value)] ?? text(value);
...
export function resolveBillLineUnit(product, requestedUnit, quantity) {
  ...
  if (units.length) {
    ...
    selected = matches[0];
    if (!selected && product.packagingMode !== "per_pack" && measure[requested]) {
      ...
      const loose = units.filter(unit => {
        ...
        return measure[target]?.[0] === measure[requested][0]
    ...
    if (!selected || !selected.id || !(Number(selected.conversionToBase) > 0)) return problem("unit_not_available");
  } else {
    ...
    if (requested !== target) {
      if (!measure[requested] || measure[requested][0] !== measure[target]?.[0]) return problem("unit_not_available");
