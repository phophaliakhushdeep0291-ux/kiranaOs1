...
export async function deleteEndpoint(req, res, next) { try { await svc.deleteWebhookEndpoint(req.shopId, req.params.id); await audit(req, "WEBHOOK_ENDPOINT_DELETED", "WebhookEndpoint", req.params.id); ok(res, { deleted: true }); } catch (e) { next(e); } }
