...
export async function tallyPosted(req, res, next) { try { const data = await svc.markTallyPosted(req.shopId, req.body.documents); await audit(req, "TALLY_VOUCHERS_POSTED", "Shop", req.shopId, { recorded: data.recorded }); ok(res, data); } catch (e) { next(e); } }
