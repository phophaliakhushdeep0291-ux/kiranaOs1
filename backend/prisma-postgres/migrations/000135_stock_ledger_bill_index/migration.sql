...
--
-- "What stock did this bill move" had no index to sit on. StockLedger carried a
-- billId column and not one index containing it, so every read keyed on a single
-- sale scanned the shop's entire movement history:
--
...
--
-- `action` is third so the two-column prefix still serves the assurance read,
-- which does not filter on it. The cost of all four grew with how long the shop
-- had been trading, not with the one bill they were asking about.
