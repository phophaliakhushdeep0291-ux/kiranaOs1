...
for (const name of ["SIM_OWNER_MOBILE", "SIM_OWNER_PASSWORD", "SIM_SHOP_ID"]) {
  if (!process.env[name]) throw new Error(`${name} is required`);
}

const client = makeClient({ deviceId: "sim-fix-recheck-results", pin: OWNER_PIN });
...
function directMetrics(data) {
  return {
    rows: data.rows?.length ?? 0,
    recordCount: data.count ?? null,
    truncated: data.truncated ?? null,
    limit: data.limit ?? null,
    newestDate: data.rows?.[0]?.date ?? null,
    oldestDate: data.rows?.at(-1)?.date ?? null,
  };
}
