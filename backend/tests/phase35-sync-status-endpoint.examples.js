...
function run() {
  ...
  mustContain('src/modules/sync/sync.routes.js', 'router.post("/ack", requireDeviceAllowedForSync(), validate(syncAckSchema), ctrl.ack)', 'applied sequence acknowledgement must be plan and device enforced');
  mustContain('src/modules/sync/sync.routes.js', 'router.get("/devices", requireDeviceActivated(), requireRole("owner", "admin"), ctrl.devices)', 'fleet sync health must be restricted to management roles');
  ...
  mustContain('contracts/api-contract.v1.json', '"path": "/api/sync/ack"', 'API contract must document monotonic sequence acknowledgement');
  mustContain('contracts/api-contract.v1.json', '"path": "/api/sync/devices"', 'API contract must document fleet sync health');
