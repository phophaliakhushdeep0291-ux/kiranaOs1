...
const productController = fs.readFileSync("src/modules/products/products.controller.js", "utf8");
const productService = fs.readFileSync("src/modules/products/products.service.js", "utf8");
const inventoryController = fs.readFileSync("src/modules/inventory/inventory.controller.js", "utf8");
...
for (const action of [
  ...
  assert.ok(productController.includes(action), `${action} audit action must be logged from product controller`);
}

assert.match(productController, /PRODUCT_RESTORED[\s\S]*metadata:[\s\S]*name:[\s\S]*category:/, "product restore audit must include product metadata");
assert.match(productController, /PRODUCT_PERMANENTLY_DELETED[\s\S]*metadata:[\s\S]*hardDelete: true/, "permanent delete audit must include hard delete metadata");
assert.match(productController, /PRODUCT_RECYCLE_BIN_EMPTIED[\s\S]*deletedCount:[\s\S]*blockedCount:/, "empty recycle bin audit must include deleted/blocked counts");
...
for (const action of ["STOCK_CORRECTED", "STOCK_DAMAGED"]) {
  assert.ok(inventoryController.includes(action), `${action} audit action must be logged from inventory controller`);
}
assert.match(inventoryController, /STOCK_CORRECTED[\s\S]*before:[\s\S]*oldStockBaseQty[\s\S]*after:[\s\S]*newStockBaseQty/, "stock correction audit must include before/after stock");
assert.match(inventoryController, /STOCK_DAMAGED[\s\S]*damageLossValue[\s\S]*oldStockBaseQty[\s\S]*newStockBaseQty/, "stock damage audit must include loss and stock metadata");
