      ...
      async upsert({ where, create, update, select }) {
        ...
        if (!counters.has(shopId)) {
          counters.set(shopId, {
            ...
            returnLastNumber: create.returnLastNumber ?? 0,
          });
        } else {
          ...
          counters.set(shopId, {
            ...
            returnLastNumber: current.returnLastNumber + (update.returnLastNumber?.increment ?? 0),
          });
        }
...
async function run() {
  ...
  assert.match(schema, /\breturnLastNumber\s+Int\b/, "BillCounter must track returnLastNumber separately");
  ...
  const firstReturn = await generateBillNo("shop-a", tx, { billType: "sales_return" });
  const secondReturn = await generateBillNo("shop-a", tx, { billType: "sales_return" });
  ...
  assert.match(firstReturn, /^RET-\d{4}-000001$/);
  assert.match(secondReturn, /^RET-\d{4}-000002$/);
