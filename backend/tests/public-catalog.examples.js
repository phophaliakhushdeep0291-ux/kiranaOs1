...
// ── Regression: STOCK MUST NEVER GATE ONLINE ORDERING ────────────────────────
// A customer order is a request, not a sale: it lands in the owner's inbox and
// moves no stock, which only moves later if the shopkeeper bills it. So the
// storefront must not consult stock at all — and every way it used to was a
// silent loss. It hid the product from the catalogue, or dropped the line, or
// answered a 409, and the shop never learnt somebody had wanted the thing.
//
// The unit maths made it worse than a policy mistake. The old check converted
// with toBaseQty(qty, product.rateUnit, product.baseUnit), and that helper knows
// eighteen units and THROWS on the rest — while six trades declare bag, bottle,
// strip, pair, plate, meter and set as primary units (business-types.ts) and the
// product form writes them straight into rateUnit. Any cart holding one of those
// died with "Unsupported unit", so nothing in it could be ordered at any stock
// level. Asserted on the source because the bug is a READ that should not exist:
// no expression in this file may consult stock again.
const publicService = fs.readFileSync(new URL("../src/modules/public/public.service.js", import.meta.url), "utf8");
const stockReads = publicService
  .split("\n")
  .filter((line) => !line.trimStart().startsWith("*") && !line.trimStart().startsWith("//"))
  .filter((line) => /\bstockBaseQty\b|\bonHandQty\b|\blowStockThreshold\b/.test(line));
assert.deepEqual(stockReads, [], "the public storefront must never read stock — an order is a request, not a sale");
assert.ok(
  !/\btoBaseQty\b/.test(publicService),
  "the public storefront must not convert quantities: toBaseQty rejects bag/strip/pair/plate and 400s the whole order",
);

// The privacy boundary is the other half of the same rule: stock must not gate
// an order AND must not leak to the guest, so it has no business in this file at
// all. (The per-field assertions above already pin what toCustomerSafeProduct
// exposes; this pins that nothing else grew a way around it.)
assert.ok(!/ORDER_QUANTITY_UNAVAILABLE/.test(publicService), "a guest must never be refused an order over a stock count");
