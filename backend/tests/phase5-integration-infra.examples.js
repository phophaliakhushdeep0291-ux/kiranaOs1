...
assert.match(runner, /--test-force-exit/, "each isolated test process must exit after its completed test report instead of holding CI open on runtime handles");
assert.match(runner, /INTEGRATION_TEST_FILE_TIMEOUT_MS/, "each integration file must have a bounded process timeout");
assert.match(runner, /run\.error\?\.code === "ETIMEDOUT"/, "a timed-out integration process must be reported explicitly");
