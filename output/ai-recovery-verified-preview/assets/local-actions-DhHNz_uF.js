import{ak as e,N as t,aI as n,P as a,Q as r,T as o,b0 as d,X as s,V as i,ar as c,bF as u,a8 as m,a9 as l,a6 as y,J as p,U as _,bG as g,bH as I,aM as f,as as w,at as h,au as E,av as A,aw as v,ax as b,Y as S,bI as P,bJ as x,aE as M,bK as O}from"./index-C-jOBSyf.js"
import{n as L}from"./local-actions-xzJjdsp_.js"
...
e&&i.has(e)&&c.has(String(m.sync_status??"").toLowerCase())&&(u=o(u+d(m)))}return o(Math.max(0,u))}const R=new Map
function B(e,t){const n=c(),a=`artha:customer-finance:${n.tenant_id}:${n.store_id}:${e}`
return(async(e,t)=>{const n=R.get(e)??Promise.resolve()
let a
const r=new Promise(e=>{a=e}),o=n.then(()=>r)
R.set(e,o),await n
try{return await t()}finally{a(),R.get(e)===o&&R.delete(e)}})(a,()=>u(a,t))}function K(e){return new Set([e?.id,e?.local_id,e?.localId,e?.server_id,e?.serverId].filter(e=>"string"==typeof e&&e.length>0))}function V(e,t){for(const n of t){const t=e[n]
...
r&&o&&(n.has(r)&&!n.has(o)&&(n.add(o),a=!0),n.has(o)&&!n.has(r)&&(n.add(r),a=!0))}}return n}async function J(e){const t=await(async e=>{const[t,n]=await Promise.all([p.getAll("customers").catch(()=>[]),p.getAll("id_mappings").catch(()=>[])]),a=t.find(t=>q(K(t),n).has(e))
return new Set([e,...q(K(a),n)])})(e),n=await p.getAll("customer_ledger").catch(()=>[])
return a(n).filter(e=>{const n=e.customerId??e.customer_id
return"string"==typeof n&&t.has(n)})}function Y(e){return B(e.customerId,()=>(async e=>{m(l,{action:"ledger_adjustment",ownerPin:e.ownerPin,entityId:e.customerId,reason:e.note})
const t=o(y(e.amount,0))
if(0===t)throw new Error("Adjustment amount cannot be zero")
const[n,a,r,d]=await Promise.all([J(e.customerId),p.getAll("customers").catch(()=>[]),p.getAll("id_mappings").catch(()=>[]),$()]),s=a.find(t=>q(K(t),r).has(e.customerId))
...
return{totalOutstanding:o(c.reduce((e,t)=>e+t.outstanding,0)),customers:c}}function ne(){return te({customers:t(G,[]),ledgerEntries:t("customer_ledger",[]),authoritative:C()})}async function ae(){const[e,n,r,o]=await Promise.all([p.getAll("customers").catch(()=>[]),p.getAll("customer_ledger").catch(()=>[]),p.getAll("id_mappings").catch(()=>[]),$()])
return te({customers:W([...t(G,[]),...e]),ledgerEntries:a([...t("customer_ledger",[]),...n]),authoritative:o,idMappings:r})}async function re(e){return(await p.getAll("customers")).map(L).find(t=>X(t).has(e))}const oe=["payments","customer_ledger","customers","local_audit_logs","sync_outbox"]
...
if(!s)throw new Error("Customer not found in local records")
const i=await J(e),c=o(_(i)),u=await de({customerId:e,customer:s,ledgerEntries:i}),l=Math.max(0,y(n.expectedOutstanding,0)),g=!0===s.balance_derived_from_local_ledger?Math.max(0,y(s.udharAmount??s.totalUdhar,0)):null,I=o(null!==u?u:null!==g?Math.max(c,g):Math.max(c,l)),N=o(Math.max(0,I))
...
for(const t of D)await e.enqueueOutboxOperation(t.auditOutbox),await e.enqueueOutboxOperation(t.paymentOutbox)}),b(G,j,1e3)
...
return B(n,()=>(async(e,t)=>{const n=(new Date).toISOString(),a=(await p.getAll("payments").catch(()=>[])).find(t=>t.id===e.paymentId||t.local_id===e.paymentId||t.server_id===e.paymentId)
