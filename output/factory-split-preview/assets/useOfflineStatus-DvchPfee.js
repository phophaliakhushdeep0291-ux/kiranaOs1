...
import{a as e,r as t}from"./deferred-runtime-G7K-NcIL.js"
import{r as a,c as i}from"./sync-status-repair-Cj3Am-YZ.js"
import{b1 as o,h as c,p as u,e as l}from"./index-CYG8yywH.js"
...
e.browserOnline===n.browserOnline&&e.backendReachable===n.backendReachable&&e.apiBaseUrl===n.apiBaseUrl&&e.error===n.error||f({...d,backendStatus:n})}let h=!1,m=null,y=null,k=null,S=null,g=null,p=null,L=!1,C=0,E=!1
function q(){0!==C&&(C=0,L&&R())}function R(){L&&(null!==g&&window.clearTimeout(g),g=window.setTimeout(()=>{g=null,(async()=>{try{const n=await T(),e=!n||n.totalBlocking>0,t=navigator.onLine&&"visible"===document.visibilityState,a=t&&c()?await B():0
...
return await T(),a.pushed}catch(i){throw await T(),i}finally{h=!1,b(!1)}}async function M(){if("undefined"!=typeof navigator&&!navigator.onLine)return
if("undefined"!=typeof document&&"visible"!==document.visibilityState)return
const n=await T()
n&&0!==n.totalBlocking&&(h||l("kirana.sync.localQueueRecovery.lastRun",3e3)&&await B({manual:!0,hydrate:!1}))}function j(n){c()&&(null!==m&&window.clearTimeout(m),m=window.setTimeout(()=>{m=null,B().catch(()=>{})},n))}function x(){q(),c()&&u({force:!0}).then(v),j(500)}function I(){u({force:!0}).then(v)}function U(n){T()
const e=n?.detail
"sync"!==e?.type&&(q(),navigator.onLine&&"visible"===document.visibilityState&&(j(450),null!==S&&window.clearTimeout(S),S=window.setTimeout(()=>{S=null,M().catch(()=>{})},900)))}function z(n){const e=n.detail
e&&"object"==typeof e&&v(e)}function K(){"visible"===document.visibilityState&&(T(),navigator.onLine&&M().catch(()=>{}))}function N(n){return w.add(n),L||"undefined"==typeof window||(L=!0,window.addEventListener("online",x),window.addEventListener("offline",I),window.addEventListener("kirana:sync-queue-updated",U),window.addEventListener("kirana:local-data-changed",U),window.addEventListener("kirana:backend-status-changed",z),document.addEventListener("visibilitychange",K),T(),c()&&u({force:!0}).then(v),("undefined"==typeof navigator||navigator.onLine)&&c()&&(y=window.setTimeout(()=>{y=null,B().catch(()=>{})},700)),k=window.setTimeout(()=>{k=null,M().catch(()=>{})},1e3),C=0,R(),p=window.setInterval(()=>{"visible"===document.visibilityState&&c()&&u().then(v)},8e3)),()=>{w.delete(n),0===w.size&&(()=>{if(L){L=!1,O+=1,d={...d,queueStatus:"checking"},window.removeEventListener("online",x),window.removeEventListener("offline",I),window.removeEventListener("kirana:sync-queue-updated",U),window.removeEventListener("kirana:local-data-changed",U),window.removeEventListener("kirana:backend-status-changed",z),document.removeEventListener("visibilitychange",K)
