...
import{aH as n,N as a,X as r,V as i,aK as o,J as s}from"./index-CYG8yywH.js"
import{a as c,g as u}from"./query-options-DZ4mp_BJ.js"
import{g as l,f as d,h as f}from"./api-DT2z0zgj.js"
import{r as m,a as y,s as p,b as g}from"./local-actions-UqKs4SNW.js"
...
function k(t){return D.has(String(t.sync_status??t.syncStatus??t.status??"").toLowerCase())}function q(t){return String(t.productId??t.product_id??"")}function U(t,e){return t.filter(t=>{const n=q(t)
return n.length>0&&e.has(n)})}function I(){return a(_,a(w,[])).map(t=>h(t))}async function B(){try{return(await s.getAll("products")).filter(t=>null==t.deletedAt&&null==t.deleted_at).map(t=>h(t))}catch{return[]}}function F(e){const a=c(e),s=I()
return t({...a,queryKey:["inventory"],initialData:a.initialData??(s.length>0?s:void 0),initialDataUpdatedAt:a.initialDataUpdatedAt??n(_),queryFn:async()=>{const t=I()
...
return e.length>0?(r(_,e),e):t}throw e}}})}function j(){return I().filter(t=>Number(t.stockBaseQty??0)<=Number(t.lowStockThreshold??0))}async function M(){return(await B()).filter(t=>Number(t.stockBaseQty??0)<=Number(t.lowStockThreshold??0))}function Q(e){const a=c(e),r=j()
...
if(o(e))return M()
throw e}}})}function T(e,n){const u=c(n),l=Number(e?.limit??50),d=()=>{const t=U(a(S,[]),(()=>{const t=[...a(w,[]),...a(_,[])]
...
e&&a.delete(e),t.id&&a.set(t.id,t)}return[...a.values()].sort((t,e)=>e.createdAt.localeCompare(t.createdAt)).slice(0,Number.isFinite(n)?n:50)})(U([...t.entries,...a],o),n.entries??[],l)
