...
import{k as a,K as s,B as n,J as i,l as r}from"./index-CYG8yywH.js"
import{b as d,s as l}from"./restaurant-api-DNRoLYPu.js"
import{b4 as o,bu as c,ax as u,U as p}from"./vendor-ui-CIi-vqR6.js"
import{C as m}from"./chip-tones-cbbqDYQQ.js"
import{u as g,l as b}from"./api-C7P5BX87.js"
import{u as f}from"./queries-DdHROHOV.js"
import{T as x,a as h}from"./table-store-BCVlyBW_.js"
import{H as w,n as y,u as j,B as v,w as N}from"./open-bills-MM61W_EO.js"
import{d as k}from"./billing-calculations-DrJWXY6g.js"
...
t>=0?a[t]={...a[t],quantity:a[t].quantity+s.quantity}:a.push(s)}return a}async function z(e){const t=await i.getSetting(w)??[],a=await i.getSetting(x)??{},s=await i.getSetting(v)
...
if(await z(t.id),n[e.id])return n[e.id]
...
await B(),d?.(),o({title:`Added to ${t.name}`,description:a.skipped.length>0?`${a.added} item${1===a.added?"":"s"} added. Not in your catalogue: ${a.skipped.join(", ")}.`:`${a.added} item${1===a.added?" is":"s are"} on the table's bill. Fire from the Tables screen.`})}catch(a){o({title:c("restaurant.guest.acceptFailed"),description:`${a instanceof Error?a.message:""} ${c("restaurant.guest.retry")}`,variant:"destructive"})}finally{R(null)}}else o({title:`No table called ${e.tableName??"that"} on this floor`,description:c("restaurant.guest.noTableHelp"),variant:"destructive"})})(e)},children:[t.jsx(p,{size:13})," ",L===e.id?"Adding…":`Add to ${e.tableName??"table"}`]})]},e.id))})]}):null]}):null}export{F as G,S as a}
