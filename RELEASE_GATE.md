...
Staff-control addendum (2026-08-11): staff create/edit/deactivate is server-authoritative; a disconnected device may read its cache but cannot claim an access change was accepted. Backend create, update, role, disable and location-scope changes write the required owner audit inside the same transaction, password changes revoke active staff sessions, and the owner account cannot be rewritten through staff management. A per-shop invitation lock keeps the plan-seat decision atomic in SQLite/tests and across PostgreSQL server instances. The isolated owner/RBAC integration passes 17/17, including five successful simultaneous Growth-plan seats, one `STAFF_LIMIT_EXCEEDED`, five initial staff rows and five creation audits; an audited deactivation then releases exactly one replacement seat without deleting the historical user. Auth-session, frontend/auth readiness and production-hardening examples pass; backend production, migration-safety (0 warnings) and release-gate checks pass. This is local ADM-001 evidence and does not replace exact-candidate CI, human approval or the other external gates.
