...
|---|---|---|
| Frontend typecheck | `cd frontend && npm run typecheck` | Local working tree passed 2026-08-08 |
| Frontend tests | `cd frontend && npm run test` | Local working tree passed 2026-08-08: 1,319 passed, 1 skipped across 208 passed files and 1 skipped file |
| Hardware bridge software/installer contract | `cd hardware-bridge && npm test` plus PowerShell parser | Local working tree passed 2026-08-08: 15/15 software tests and release PowerShell syntax. Native .NET setup compilation, signed workflow artifact, clean-Windows install and physical printer runs remain external evidence. |
| Frontend production build/security | `cd frontend && npm run build && npm run security:check` | Local working tree passed 2026-08-08: startup 859.0 kB raw / 256.7 kB gzip across 5 files; total JavaScript 3,952.8 kB raw / 1,155.9 kB gzip; largest offline shop payload restaurant 2,425.8 kB raw / 724.2 kB gzip; fixed bundle budgets and production-app checks passed. Translation parity also passes for 1,085 keys across 6 modules. The +1.3 kB gzip on startup versus the previous run is the English half of the billing and first-run-setup keys added on 2026-08-08; Hindi remains lazily loaded, and the kirana startup payload measured 320,117 gzip bytes across 6 assets with no vertical page chunks. |
| Backend tests | `cd backend && npm test` | Full local baseline passed 2026-07-18; current 2026-08-08 focused billing and restaurant suites pass, including portion/add-on money and stock regressions |
| Backend production check | `cd backend && npm run prod:check` | Local working tree passed 2026-08-08 after packaging and current-plan checks |
...
|---|---|---|
| Cash, split, udhar and GST/estimate checkout | BILL-001..004 / MQA-BILL-01 | Partially verified. Live 390px cash and ₹20 cash + ₹30 UPI split bills persist, sync and appear once with distinct payment modes; a separate estimate uses the EST series and remains separated in history. Restaurant checkout also proves a ₹590 Large portion plus ₹85 add-on persists and syncs as exactly ₹675, with matching backend stock consumption. Exact inclusive-GST accounting is covered by the RPT-001..002 release fixture. Dedicated Udhar checkout QA remains open. |
...
| 375/390/430/768 mobile matrix | QUAL-003 / all MQA flows | Overview routes for Billing, Products, Inventory, Purchases, Reports, Settings and Sync pass overflow/runtime smoke; Customers has separate multi-width evidence. Premium Reports and Daily Closing layouts pass all four target widths. Restaurant add-on configuration, ₹675 checkout and final bill detail now pass live 375x667, 390x844, 430x932 and 768x1024 geometry: document and each active dialog match the viewport, phone item cards and the 768px table wrapper have no internal overflow, and every measured active control is at least 44px after dialog motion settles. Retained artifacts: `docs/evidence/mobile-restaurant-addon/`. Other core transactional flows remain open. |
