...
async function registerSession(client) {
  ...
  await client.evaluate(`(async()=>{const apiUrl=${JSON.stringify(API_URL)},deviceId="mobile_matrix_${runId}";const response=await fetch(apiUrl+"/auth/register",{method:"POST",headers:{"content-type":"application/json","x-device-id":deviceId},body:JSON.stringify({shopName:"Mobile Matrix QA",ownerName:"QA Owner",city:"Jaipur",address:"Automated QA",mobile:${JSON.stringify(mobile)},password:"Test@12345",ownerPin:"2468"})});const json=await response.json();if(!response.ok)throw new Error("Registration failed: "+JSON.stringify(json));const auth=json.data??json;localStorage.setItem("kiranaApiBaseUrl",apiUrl);localStorage.setItem("kiranaos_device_id",deviceId);localStorage.setItem("kirana-os:device-id:v1",deviceId);localStorage.setItem("kiranaos.auth.session.v1",JSON.stringify({accessToken:auth.accessToken??auth.token,refreshToken:auth.refreshToken,user:auth.user,shop:auth.shop}));sessionStorage.setItem("kiranaos.security.sessionStarted.v1",String(Date.now()));return true})()`);
}
...
async function auditPage(client, qaId, route, width, height) {
  ...
  assert(!metrics.desktopAsideVisible, `${qaId} ${width}px shows desktop sidebar`);
