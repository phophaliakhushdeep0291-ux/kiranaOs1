...
async function main() {
  ...
  try {
    ...
    const audit = await client.evaluate(`(() => { const text=document.body.innerText; const card=(label)=>[...document.querySelectorAll('article')].map(node=>node.innerText.replace(/\\s+/g,' ')).find(value=>value.startsWith(label))||''; return {customers:card('Total Customers'),outstanding:card('Total Outstanding'),received:card('Received This Period'),selected:text.includes('Ramesh Sharma'),amountDue:text.includes('₹24,850'),ledger:text.includes('INV-01562'),sparks:document.querySelectorAll('.recharts-area-area').length}; })()`);
    ...
    const guardTriggered=await client.evaluate(`(() => { const input=[...document.querySelectorAll('input')].find(node=>node.placeholder==='Enter amount'); const button=[...document.querySelectorAll('button')].find(node=>node.textContent?.trim()==='Collect Payment'); if(!input||!button)return false; const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; setter.call(input,'24851'); input.dispatchEvent(new Event('input',{bubbles:true})); button.click(); return true; })()`);
    assert(guardTriggered,'Could not exercise overpayment guard'); await waitForPage(client,"document.body.innerText.includes('Amount exceeds outstanding udhar')");
