...
describe("daily closing cash split UI", () => {
  it("does not double count old udhar in the cash vs UPI split", () => {
    ...
    expect(source).toContain("(report?.cashReceived ?? 0) + (report?.upiReceived ?? 0)");
  ...
  it("labels sales, old udhar, and supplier cash separately", () => {
    ...
    expect(source).toContain('FlowRow label="UPI / digital sales" value={fmt(report?.upiSales)}');
    expect(source).toContain('FlowRow label="Supplier cash paid" value={fmt(report?.purchaseCashPaid)}');
    expect(source).toContain('FlowRow label="Supplier UPI/bank paid" value={fmt(report?.purchaseUpiPaid)}');
  });
});
