...
describe("transactional backup restore UI", () => {
  ...
  // Restore-safety wording moved to the dictionary; assert it there and assert
  // the screen renders the matching key.
  const settingsEn = fs.readFileSync(path.resolve("src/features/core/settings/translations/settings-pages.ts"), "utf8");
  ...
  it("refuses restore while this device has unresolved sync work", () => {
    ...
    expect(settingsEn).toContain("Resolve every pending, failed, or conflicting local change");
    expect(page).toContain("settings.sync.resolveFirst");
  });
