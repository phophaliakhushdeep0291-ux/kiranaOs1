...
const paymentModes = (t: Translate): Record<PaymentMode, { label: string; className: string }> => ({
  ...
  udhar:  { label: t("billing.pay.udhar"),  className: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-900" },
...
export function PaymentBadge({ mode, compact = false, className }: PaymentBadgeProps) {
  ...
  const config = paymentModes(t)[key] ?? paymentModes(t).other;
  return (
    <span
      className={cn(
