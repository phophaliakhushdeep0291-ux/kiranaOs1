...
export default function CustomerDetailPage() {
  ...
  return (
    <div className="app-page-shell w-full max-w-none space-y-5">
      ...
      />
      <Dialog open={adjustOpen} onOpenChange={setAdjustOpen}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{t("customers.account.manualAdjustment")}</DialogTitle></DialogHeader><div className="space-y-4"><p className="text-sm text-muted-foreground">{t("customers.account.adjustmentHint")}</p><div><Label>{t("customers.account.amountRequired")}</Label><Input type="number" inputMode="decimal" step="0.01" className="mt-1" value={adjust.amount} onChange={(event) => setAdjust((form) => ({ ...form, amount: event.target.value }))} /></div><div><Label>{t("customers.account.ownerPinRequired")}</Label><Input type="password" className="mt-1" value={adjust.ownerPin} onChange={(event) => setAdjust((form) => ({ ...form, ownerPin: event.target.value }))} /></div><div><Label>{t("customers.account.reason")}</Label><Textarea className="mt-1" value={adjust.note} onChange={(event) => setAdjust((form) => ({ ...form, note: event.target.value }))} /></div></div><div className="flex justify-end gap-2 pt-2"><Button variant="outline" onClick={() => setAdjustOpen(false)}>{t("customers.account.cancel")}</Button><Button disabled={saving} onClick={() => void saveAdjustment()}>{saving ? t("customers.detail.saving") : t("customers.detail.saveCorrection")}</Button></div></DialogContent></Dialog>
    </div>
  );
}
