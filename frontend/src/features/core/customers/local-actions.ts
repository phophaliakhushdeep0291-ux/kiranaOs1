...
function toCustomer(data: CustomerInput, id = createLocalId("customer"), existing?: Customer): Customer {
  ...
  return normaliseLocalCustomer({
    ...
    promiseToPayDate: data.promiseToPayDate !== undefined ? data.promiseToPayDate : existing?.promiseToPayDate ?? null,
    udharLimit: data.udharLimit !== undefined ? data.udharLimit : existing?.udharLimit ?? null,
