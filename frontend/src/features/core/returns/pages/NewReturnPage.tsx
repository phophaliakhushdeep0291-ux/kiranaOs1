...
import { useListCustomers } from "@/features/core/customers/queries";
import { useListProducts } from "@/features/core/products/queries";
import { cancelPurchaseReturn, listPurchaseReturns, type PurchaseReturn } from "@/features/core/purchases/purchase-orders-api";
    ...
    <PageShell className="space-y-4 bg-white p-4 sm:p-5 2xl:p-6">
      <section className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between 2xl:fixed 2xl:right-[276px] 2xl:top-[18px] 2xl:z-[60] 2xl:w-auto 2xl:flex-row 2xl:gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="h-10 min-w-[230px] justify-between rounded-[8px] border-[#dfe7f2] bg-white px-3 text-[12px] font-semibold text-[#24385f]">
                <span className="inline-flex items-center gap-2"><CalendarDays size={14} className="text-[var(--brand)]" />{format(new Date(`${from}T00:00:00`), "dd MMM yyyy")} - {format(new Date(`${to}T00:00:00`), "dd MMM yyyy")}</span>
                <ChevronDown size={13} />
              </Button>
            </PopoverTrigger>
            <PopoverContent align="end" className="w-[320px] rounded-[8px] border-[#dfe7f2] p-3">
              <div className="mb-3 grid grid-cols-3 gap-1.5">
                ...
                <button className="rounded-[6px] bg-[var(--brand-soft)] px-2 py-2 text-[11px] font-bold text-[var(--brand)]" onClick={() => applyPreset(6)}>7 days</button>
          ...
          <Button onClick={() => tab === "purchase" ? navigate("/purchase-bills") : setBuilderOpen(true)} className="h-10 gap-2 rounded-[8px] bg-[var(--brand)] px-5 text-[12px] font-bold text-white shadow-[0_8px_18px_rgba(7,95,255,0.2)] hover:bg-[var(--brand-strong)]"><Plus size={15} />{tab === "purchase" ? "Supplier Return" : "New Return"}</Button>
        </div>
      </section>

      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        <MetricCard label="Total Returns" value={inr(metrics.total)} previous={previousMetrics.total} current={metrics.total} color="var(--brand)" icon={<ShoppingBag size={16} />} iconClass="bg-[var(--brand-soft)] text-[var(--brand)]" spark={spark.map((row) => row.total)} />
        ...
        <MetricCard label="Credit Issued (Udhar)" value={inr(metrics.credit)} previous={previousMetrics.credit} current={metrics.credit} color="var(--brand)" icon={<CreditCard size={16} />} iconClass="bg-[var(--brand-soft)] text-[var(--brand)]" spark={spark.map((row) => row.credit)} />
      </section>
      ...
      <p className="mx-auto flex max-w-xl items-center justify-center gap-2 rounded-full bg-[#f7f9fc] px-4 py-2 text-center text-[10px] font-medium text-[#687792]"><CheckCircle2 size={13} className="text-[var(--brand)]" />{tab === "purchase" ? "Supplier returns remove branch stock, preserve batch traceability and record the credit or refund settlement." : "Sales returns affect stock. Resellable items are added back to inventory automatically."}</p>
...
function MetricCard({ label, value, current, previous, color, icon, iconClass, spark, positiveIsBad = false }: { label: string; value: string; current: number; previous: number; color: string; icon: ReactNode; iconClass: string; spark: number[]; positiveIsBad?: boolean }) {
  ...
  return <article className={cn(PANEL, "min-h-[150px] p-4")}><div className="flex items-center gap-3"><span className={cn("grid h-9 w-9 place-items-center rounded-[9px]", iconClass)}>{icon}</span><p className="text-[11px] font-semibold text-[#34486e]">{label}</p></div><p className="mt-3 text-[22px] font-black leading-none text-[var(--brand-ink)]">{value}</p><div className="mt-2 flex items-center gap-1 text-[9.5px]"><span className={cn("inline-flex items-center gap-0.5 font-bold", change === 0 ? "text-[#70809a]" : bad ? "text-[#ff334d]" : "text-[#10a948]")}>{change >= 0 ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}{Math.abs(change)}%</span><span className="text-[#7a879f]">vs last period</span></div><div className="mt-1.5 h-7"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}><defs><linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity={0.25} /><stop offset="100%" stopColor={color} stopOpacity={0} /></linearGradient></defs><Area type="monotone" dataKey="value" stroke={color} strokeWidth={1.7} fill={`url(#${gradientId})`} dot={{ r: 1.5, fill: "white", stroke: color, strokeWidth: 1.2 }} isAnimationActive={false} /></AreaChart></ResponsiveContainer></div></article>;
}

function TabButton({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon: ReactNode; children: ReactNode }) {
  return <button type="button" onClick={onClick} className={cn("inline-flex h-10 min-w-[160px] items-center justify-center gap-2 rounded-[8px] border px-4 text-[11px] font-bold transition-colors", active ? "border-[var(--brand)] bg-[#f4f7ff] text-[var(--brand)] shadow-[0_4px_12px_rgba(7,95,255,0.06)]" : "border-[#dfe7f2] bg-white text-[#405273] hover:bg-[#f8faff]")}>{icon}{children}</button>;
}
