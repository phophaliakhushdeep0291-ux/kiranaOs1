...
function MetricCard({ label, value, current, previous, color, icon, iconClass, spark, positiveIsBad = false }: { label: string; value: string; current: number; previous: number; color: string; icon: ReactNode; iconClass: string; spark: number[]; positiveIsBad?: boolean }) {
  ...
  return <article className={cn(PANEL, "min-h-[150px] p-4")}><div className="flex items-center gap-3"><span className={cn("grid h-9 w-9 place-items-center rounded-[9px]", iconClass)}>{icon}</span><p className="text-[11px] font-semibold text-[#34486e]">{label}</p></div><p className="mt-3 text-[22px] font-black leading-none text-[var(--brand-ink)]">{value}</p><div className="mt-2 flex items-center gap-1 text-[9.5px]"><span className={cn("inline-flex items-center gap-0.5 font-bold", change === 0 ? "text-[#70809a]" : bad ? "text-[#ff334d]" : "text-[var(--success-ink)]")}>{change >= 0 ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}{Math.abs(change)}%</span><span className="text-[#7a879f]">vs last period</span></div><div className="mt-1.5 h-7"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}><defs><linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity={0.25} /><stop offset="100%" stopColor={color} stopOpacity={0} /></linearGradient></defs><Area type="monotone" dataKey="value" stroke={color} strokeWidth={1.7} fill={`url(#${gradientId})`} dot={{ r: 1.5, fill: "white", stroke: color, strokeWidth: 1.2 }} isAnimationActive={false} /></AreaChart></ResponsiveContainer></div></article>;
}

function TabButton({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon: ReactNode; children: ReactNode }) {
  return <button type="button" onClick={onClick} aria-pressed={active} className={cn("inline-flex h-11 min-w-[160px] items-center justify-center gap-2 rounded-[8px] border px-4 text-[11px] font-bold transition-colors lg:mouse:h-10", active ? "border-[var(--brand)] bg-[#f4f7ff] text-[var(--brand)] shadow-[0_4px_12px_rgba(7,95,255,0.06)]" : "border-[#dfe7f2] bg-white text-[#405273] hover:bg-[#f8faff]")}>{icon}{children}</button>;
}
