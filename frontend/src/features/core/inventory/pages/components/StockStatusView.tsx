...
// Quantity, value and the low/out verdict all come from inventoryStockRows, which
// respects each product's selling-unit conversion (a "packet" of 6, a 500 g pack)
...
export function StockStatusView({ mode }: { mode: "in" | "out" }) {
  ...
  const [movePreselectUnit, setMovePreselectUnit] = useState<string | undefined>(undefined);
  ...
  // One row per PACK SIZE for products that count each size separately: "which
  ...
  const rows = useMemo(() => {
    ...
    return scoped
      ...
      .filter((row) => {
        ...
        if (mode === "out" && statusF === "all") return row.isOut;
        if (statusF === "all") return true;
        if (statusF === "low") return row.isLow;
        if (statusF === "in") return row.quantity > 0 && !row.isLow;
  ...
  const stats = useMemo(() => {
    const allRows = all.flatMap((p) => inventoryStockRows(p));
    const inStock = allRows.filter((row) => row.quantity > 0);
    const outOfStock = allRows.filter((row) => row.isOut);
    const totalQty = inStock.reduce((s, row) => s + row.quantity, 0);
    const totalValue = inStock.reduce((s, row) => s + row.value, 0);
    const cats = new Set(outOfStock.map((row) => (row.item.category ?? "general").trim() || "general"));
    const lowCount = allRows.filter((row) => row.isLow).length;
    return { inStockCount: inStock.length, outOfStockCount: outOfStock.length, totalQty: Math.round(totalQty), totalValue, cats: cats.size, lowCount, rowCount: allRows.length };
  }, [all]);
  ...
  function openMove(m: "in" | "out", productId?: string, unitCode?: string) {
    setMoveMode(m);
    setMovePreselect(productId);
    // Opening from a size's row lands on that size, so the shopkeeper never has to
    // re-pick the pack they just clicked.
    setMovePreselectUnit(unitCode);
    setMoveOpen(true);
  }
  ...
  return (
    ...
    >
            ...
            <tbody>
              {(products.isLoading || inventory.isLoading) && rows.length === 0 ? (
                ...
                pagedRows.map((row) => {
                  ...
                  const packBarcode = row.unit?.barcode ?? p.barcode ?? p.sku ?? "—";
                  return (
                    <tr key={row.key} className="border-b border-[#f1f4f8] last:border-0 transition-colors hover:bg-[#f9fbfe]">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            {/* The size is part of the row's identity here — two rows
                                of one product differ only by their packaging. */}
                            {row.unit ? <p className="truncate text-[11px] font-bold text-[var(--brand)]">{row.label}</p> : brand ? <p className="truncate text-[11px] text-[#8a97ad]">{brand}</p> : null}
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="flex justify-center">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <button className="grid h-8 w-8 place-items-center rounded-lg text-[#536383] transition-colors hover:bg-[#f1f4f8] data-[state=open]:bg-[#eef4ff] data-[state=open]:text-[var(--brand)]" aria-label={`Actions for ${p.name}${row.unit ? ` ${row.label}` : ""}`}>
      ...
      <StockMovementDialog mode={moveMode} open={moveOpen} onOpenChange={setMoveOpen} initialProductId={movePreselect} initialUnitCode={movePreselectUnit} width={panelWidth} onResizeStart={onResizeStart} />
    </div>
  );
}
