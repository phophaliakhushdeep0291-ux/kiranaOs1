            ...
            <div className="border-b border-slate-100 p-5">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {session.status === "counting" && <><Button variant="outline" disabled={!dirty.size || saveMutation.isPending} onClick={() => saveMutation.mutate()}><Save size={15} /> {saveMutation.isPending ? t("inventory.counts.saving") : `${t("inventory.counts.saveAction")}${dirty.size ? ` (${dirty.size})` : ""}`}</Button><Button disabled={effectiveRemaining > 0 || submitMutation.isPending || saveMutation.isPending} onClick={() => submitMutation.mutate()}><ClipboardCheck size={15} /> {submitMutation.isPending ? t("inventory.counts.submitting") : t("inventory.counts.submitForReview")}</Button></>}
