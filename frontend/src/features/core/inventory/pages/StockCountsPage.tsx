    ...
    <div className="space-y-5 pb-10">
      <section className="overflow-hidden rounded-[24px] border border-indigo-100 bg-[radial-gradient(circle_at_top_right,#dbeafe_0,transparent_38%),linear-gradient(135deg,#071a3b,#163d85)] p-6 text-white shadow-[0_24px_60px_rgba(15,49,104,0.18)] sm:p-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          ...
          <Button className="bg-white font-black text-blue-700 hover:bg-blue-50" onClick={() => setStartOpen(true)} disabled={Boolean(active)}><Play size={16} /> {t("inventory.counts.start")}</Button>
        </div>
      </section>
          ...
          {session && <>
            <div className="border-b border-slate-100 p-5">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-black text-slate-900">{session.name}</h2><span className={cn("rounded-full px-2.5 py-1 text-[10px] font-black uppercase ring-1", statusStyle[session.status])}>{session.status}</span>{session.blindCount && <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-black text-violet-700 ring-1 ring-violet-200"><EyeOff size={11} /> {t("inventory.counts.blind")}</span>}</div><p className="mt-1 text-xs text-slate-500">{session.location.name} · started {new Date(session.createdAt).toLocaleString("en-IN")}</p></div>
                <div className="flex flex-wrap gap-2">
                  {(session.status === "counting" || session.status === "review") && <Button variant="outline" className="text-rose-700 hover:bg-rose-50 hover:text-rose-800" onClick={() => { setApprovalError(null); setApproval("cancel"); }}><ArchiveX size={15} /> {t("inventory.cancel")}</Button>}
                  ...
                  {session.status === "review" && <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => { setApprovalError(null); setApproval("apply"); }}><ShieldCheck size={15} /> {t("inventory.counts.apply")}</Button>}
                </div>
              </div>
