...
import { baseUnitFor, round2, sellingUnitCode, sellingUnitConversion, sellingUnitName, UNITS } from "../product-pricing";
...
export function ProductFormPanel({
  ...
  function addAlternatePack() {
    ...
    const packMrp = Number(extraPack.mrp);
    if (!(size > 0) || !(price > 0)) {
      toast({
    ...
    const ceiling = packMrp > 0 ? packMrp : packCeiling(sellingUnitConversion(size, extraPack.packSizeUnit));
  ...
  function updatePackField(unitCode: string, field: "onHandQty" | "lowStockThreshold" | "maximumPrice", raw: string) {
