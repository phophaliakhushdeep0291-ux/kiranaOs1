      ...
      <div className="mt-3.5 overflow-hidden rounded-[14px] border border-[#e6ecf4] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        <div className="space-y-2.5 p-2.5 lg:hidden">
          {products.isLoading && rows.length === 0 ? (
            ...
            const cardPackCounts = tracked && product.packagingMode === "per_pack" && cardPacks.length > 1
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      {!tracked ? <span className="text-xs text-slate-500">{t("inventory.stock.notCounted")}</span> : outOfStock ? <StatusPill tone="rose">{t("products.badge.outOfStockShort")}</StatusPill> : low ? <StatusPill tone="amber">{t("products.badge.lowStockShort")}</StatusPill> : <StatusPill tone="emerald">{stock} available</StatusPill>}
                    </div>
            ...
            <tbody>
              {products.isLoading && rows.length === 0 ? (
                ...
                pagedRows.map((product) => {
                  ...
                  const perPackCounts = tracked && product.packagingMode === "per_pack" && packUnits.length > 1
                    ...
                    <tr key={product.id} className={`border-b border-[#f1f4f8] last:border-0 transition-colors hover:bg-[#f9fbfe] ${selectedIds.has(product.id) ? "bg-[#f3f8ff]" : ""}`} data-testid={`row-product-${product.id}`}>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {!tracked ? (
                            <span className="text-xs text-slate-500">{t("inventory.stock.notCounted")}</span>
                          ) : outOfStock ? (
...
function Pagination({ page, totalPages, onChange }: { page: number; totalPages: number; onChange: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5 lg:mouse:gap-1">
      <button onClick={() => onChange(Math.max(1, page - 1))} disabled={page <= 1} className="grid h-11 w-11 place-items-center rounded-lg border border-[#e3eaf3] text-[#536383] transition-colors hover:bg-[#f7f9fd] disabled:opacity-40 lg:mouse:h-8 lg:mouse:w-8" aria-label={t("products.previousPage")}>
        <ChevronLeft size={15} />
      </button>
      {pages.map((p, i) =>
        p === "…" ? (
          ...
          <button
            ...
            className={`grid h-11 min-w-11 place-items-center rounded-lg px-2 text-[12px] font-bold transition-colors lg:mouse:h-8 lg:mouse:min-w-8 ${
      ...
      <button onClick={() => onChange(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="grid h-11 w-11 place-items-center rounded-lg border border-[#e3eaf3] text-[#536383] transition-colors hover:bg-[#f7f9fd] disabled:opacity-40 lg:mouse:h-8 lg:mouse:w-8" aria-label={t("products.nextPage")}>
