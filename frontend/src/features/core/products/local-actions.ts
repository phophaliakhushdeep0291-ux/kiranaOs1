...
function toProduct(data: ProductInput, id = createLocalId("product"), existing?: Product): Product {
  ...
  return {
    ...
    restaurantItemType: normalized.restaurantItemType ?? existing?.restaurantItemType ?? null,
    stockTrackingEnabled: normalized.restaurantItemType !== "prepared",
    trackStock: normalized.restaurantItemType !== "prepared",
...
export async function patchProductLocalFirst(id: string, data: Partial<ProductInput>, ownerPin?: string, reason?: string): Promise<Product> {
  ...
  const fallbackInput: ProductInput = {
    ...
    restaurantItemType: existing?.restaurantItemType ?? null,
    stockTrackingEnabled: existing?.restaurantItemType !== "prepared",
    trackStock: existing?.restaurantItemType !== "prepared",
