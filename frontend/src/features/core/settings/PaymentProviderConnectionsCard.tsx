...
export function PaymentProviderConnectionsCard() {
  ...
  return <>
    <Card>
      ...
      <div className="px-5 pb-5">
        {connectionsQ.isLoading ? <div className="flex items-center gap-2 py-6 text-sm text-slate-500"><Loader2 size={16} className="animate-spin" />{t("settings.payments.loading")}</div> : !razorpay ? <div className="rounded-xl border border-dashed p-5 text-sm text-slate-600">{t("settings.payments.none")}</div> : <div className="space-y-3 rounded-xl border p-4">
          <div className="flex flex-wrap items-center gap-2"><strong>{t("settings.payments.razorpay")}</strong><Badge tone={razorpay.environment === "live" ? "red" : "violet"}>{t(`settings.payments.environment.${razorpay.environment}`)}</Badge><Badge tone={razorpay.selected ? "green" : razorpay.status === "verified" ? "blue" : "amber"}>{razorpay.selected ? t("settings.payments.selected") : t(`settings.payments.status.${razorpay.status}`)}</Badge></div>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{t("settings.payments.connectTitle")}</DialogTitle><DialogDescription>{t("settings.payments.connectHelp")}</DialogDescription></DialogHeader><div className="space-y-4"><Fld label={t("settings.payments.environment")}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={environment} onChange={(event) => setEnvironment(event.target.value as "test" | "live")}><option value="test">{t("settings.payments.environment.test")}</option><option value="live">{t("settings.payments.environment.live")}</option></select></Fld><Fld label={t("settings.payments.keyId")}><Input autoComplete="off" value={keyId} onChange={(event) => setKeyId(event.target.value)} /></Fld><Fld label={t("settings.payments.keySecret")}><Input type="password" autoComplete="new-password" value={keySecret} onChange={(event) => setKeySecret(event.target.value)} /></Fld><Fld label={t("settings.payments.webhookSecret")}><Input type="password" autoComplete="new-password" value={webhookSecret} onChange={(event) => setWebhookSecret(event.target.value)} /></Fld></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>{t("settings.integrations.cancel")}</Button><Button disabled={!keyId || !keySecret || !webhookSecret} onClick={save}>{t("settings.payments.continue")}</Button></DialogFooter></DialogContent></Dialog>
