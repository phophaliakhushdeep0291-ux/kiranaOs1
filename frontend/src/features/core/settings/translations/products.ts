// Products catalogue strings — list, filters, the add/edit form, bulk edit and
// CSV migration.
//
// Same register rule as billing: words the shop already says in English ("स्टॉक",
// "बारकोड", "कैटेगरी", "MRP", "GST") stay as loanwords in Devanagari rather than
// being replaced by textbook Hindi nobody uses at a counter.
export const productsEn = {
  // ── List page: stats, search, filters ──
  "products.stats.total": "Total Products",
  "products.stats.totalHint": "Active listings",
  "products.stats.lowStock": "Low Stock",
  "products.stats.lowStockHint": "Needs attention",
  "products.stats.outOfStock": "Out of Stock",
  "products.stats.outOfStockHint": "Unavailable",
  "products.stats.categories": "Categories",
  "products.stats.categoriesHint": "Active categories",
  "products.search.placeholder": "Search by product name, barcode or SKU",
  "products.filter.allCategories": "All Categories",
  "products.filter.categoryLabel": "Filter products by category",
  "products.filter.title": "Filters",
  "products.filter.clearAll": "Clear all",
  "products.filter.stock": "Stock",
  "products.filter.itemType": "Item type",
  "products.filter.status": "Status",
  "products.filter.clear": "Clear",
  "products.filter.packed": "Packed",
  "products.filter.loose": "Loose",
  "products.filter.active": "Active",
  "products.filter.inactive": "Inactive",
  "products.filter.general": "General",

  // ── List page: table ──
  "products.list.loading": "Loading products…",
  "products.list.emptyTitle": "No products found",
  "products.list.emptyHintFilters": "Add a product or clear the current filters.",
  "products.list.emptyStartTitle": "Your catalogue is empty",
  "products.list.emptyStartHint": "Typing in every product takes hours. Start from the ready-made list for your shop type, bring in a spreadsheet you already have, or add them one at a time.",
  "products.list.emptyStarterCta": "Use the ready-made list",
  "products.list.emptyImportCta": "Import a spreadsheet",
  "products.list.emptyAddCta": "Add one product",
  "products.list.emptyHintCatalogue": "Add a product or clear filters to see your catalogue.",
  "products.list.selectAll": "Select all products on this page",
  "products.col.product": "Product",
  "products.col.category": "Category",
  "products.col.skuBarcode": "SKU / Barcode",
  "products.col.unit": "Unit",
  "products.col.mrp": "MRP",
  "products.col.costPrice": "Cost Price",
  "products.badge.needsDetails": "Needs details",
  "products.col.sellingPrice": "Selling Price",
  "products.col.stock": "Stock",
  "products.col.action": "Action",
  "products.badge.outOfStock": "Out of Stock",
  "products.badge.lowStock": "Low Stock",
  "products.badge.inStock": "In Stock",
  "products.badge.outOfStockShort": "Out of stock",
  "products.badge.lowStockShort": "Low stock",
  "products.badge.preparedToOrder": "Prepared to order",
  "products.rowsPerPage": "Rows per page",
  "products.previousPage": "Previous page",
  "products.nextPage": "Next page",

  // ── Row actions ──
  "products.action.edit": "Edit product",
  "products.action.duplicate": "Duplicate as new size",
  "products.action.customerPricing": "Customer pricing",
  "products.action.printLabel": "Print label",
  "products.action.recycle": "Recycle product",
  "products.action.moveToRecycleBin": "Move product to recycle bin",
  "products.action.moveToRecycleBinShort": "Move to recycle bin",
  "products.action.createProduct": "Create product",
  "products.action.addNewProduct": "Add new product",

  // ── Toasts ──
  "products.list.export": "Export",
  "products.toast.exported": "Catalogue downloaded",
  "products.toast.exportedDetail": "{count} products written in the same format Import reads, ready for another shop.",
  "products.toast.exportEmpty": "There are no products to export yet",
  "products.toast.added": "Product added",
  "products.toast.updated": "Product updated",
  "products.toast.savedOffline": "Product saved offline",
  "products.toast.updatedOffline": "Product updated offline",
  "products.toast.willSync": "It will sync to the cloud automatically when you're back online.",
  "products.toast.changesWillSync": "Changes will sync when you're back online.",
  "products.toast.saveFailed": "Could not save product",
  "products.toast.updateFailed": "Could not update product",
  "products.toast.checkRequired": "Check required fields.",
  "products.toast.recycledLocally": "Product moved to recycle bin locally",
  "products.toast.draftPrepared": "Product draft prepared",
  "products.toast.editPrepared": "Product edit prepared",
  "products.toast.formUpdated": "Product form updated",
  "products.toast.voiceFilled": "Voice assistant filled the form. Review pricing/stock, then save locally.",
  "products.toast.copyReady": "Copy ready",
  "products.toast.copyReadyHint": "Set the pack size, price and opening stock for the new size, then save.",
  "products.toast.popupBlocked": "Pop-up blocked",
  "products.toast.popupBlockedLabels": "Allow pop-ups to print labels.",
  "products.toast.popupBlockedPrice": "Allow pop-ups for this site to print price labels.",
  "products.toast.permissionDenied": "Permission denied",
  "products.toast.ownerApprovalRequired": "Owner approval required",
  "products.toast.ownerPasswordRequired": "Owner password required",
  "products.toast.priceNeedsApproval": "This product price needs owner approval before saving.",
  "products.toast.approveUpdate": "Approve update",

  // ── Add / edit form ──
  "products.form.addTitle": "Add New Product",
  "products.form.addSubtitle": "Fill in the details to add a new product",
  "products.form.editTitle": "Edit Product",
  "products.form.editSubtitle": "Update the details of this product",
  "products.form.close": "Close",
  "products.form.basicInfo": "Basic Information",
  // The trade the form has shaped itself around, shown above the fields.
  "products.form.optimizedFor": "Optimized for {trade}",
  "products.form.packedItem": "Packed Item",
  "products.form.looseItem": "Loose Item",
  "products.form.category": "Category",
  "products.form.select": "Select",
  "products.form.packSizeHint": "Pack size used for billing and stock",
  "products.form.quickSellingUnit": "Quick selling unit",
  "products.form.exactPack": "Exact pack",
  "products.form.exactPackPlaceholder": "e.g. 500",
  "products.form.hsn": "HSN (Optional)",
  "products.form.hsnPlaceholder": "e.g. 25010010",
  "products.form.aliasesTitle": "Aliases & Local Names",
  "products.form.aliasesLabel": "Alternate names (comma separated)",
  "products.form.aliasesPlaceholder": "namak, salt, साल्ट",
  "products.form.pricing": "Pricing",
  "products.form.mrp": "MRP (₹)",
  "products.form.mrpZeroHint": "No MRP means no price ceiling — billing will accept any price for this item.",
  "products.form.costPrice": "Cost Price (₹)",
  "products.form.minPrice": "Min Price (₹)",
  "products.form.sellingPrice": "Selling Price (₹)",
  "products.form.amountPlaceholder": "0.00",
  "products.form.gstRate": "GST Rate",
  "products.form.avgCost": "Avg. cost",
  "products.form.margin": "Margin",
  "products.form.otherPackSizes": "Other pack sizes",
  "products.form.otherPackSizesHint": "Optional: sell the same stock as 500 g, 1 kg, carton, etc.",
  "products.form.stockCounting": "Stock counting",
  "products.form.oneSharedStock": "One shared stock",
  "products.form.oneSharedStockHint": "All sizes draw on the same stock. Best for loose goods.",
  "products.form.countEachSize": "Count each size",
  "products.form.countEachSizeHint": "Each size has its own quantity and reorder alert.",
  "products.form.inStock": "In stock",
  "products.form.alertBelow": "Alert below",
  "products.form.soldAs": "Sold as",
  "products.form.soldAsTitle": "Sold As",
  "products.form.sellingPriceRs": "Selling price (₹)",
  "products.form.packCost": "Cost price for this pack (₹)",
  "products.form.packCostHint": "What one of THIS pack costs you. Blank = the product cost scaled to this pack size.",
  "products.form.packMrp": "MRP for this pack (₹)",
  "products.form.packMrpHint": "Blank = the product MRP scaled to this pack size.",
  "products.form.packPriceAboveMrp": "Price is above this pack's MRP",
  "products.form.packPriceAboveMrpHint": "Lower the selling price or raise this pack's MRP.",
  "products.form.measure": "Measure",
  "products.form.openingQuantity": "Opening quantity (Optional)",
  "products.form.packBarcode": "Pack barcode (Optional)",
  "products.form.packBarcodeTaken": "That barcode is already on another pack",
  "products.form.packBarcodeTakenHint": "One barcode can only name one pack, or a scan at the counter picks the wrong size.",
  "products.form.packBarcodePlaceholder": "Barcode for this size",
  "products.form.addPack": "Add pack to product",
  "products.form.addSize": "Add size",
  "products.form.packDetailsHint": "Enter what the pack contains and its selling price.",
  "products.form.stockInventory": "Stock & Inventory",
  "products.form.restaurantItemTypeTitle": "Restaurant item type",
  "products.form.restaurantItemTypeLabel": "How this item is used",
  "products.form.restaurantPreparedOption": "Prepared dish — ingredients are stock",
  "products.form.restaurantPackagedOption": "Packaged item — count each bottle or packet",
  "products.form.restaurantIngredientOption": "Kitchen ingredient — keep off the guest menu",
  "products.form.restaurantPreparedHint": "Use this for cooked food and drinks. Sales consume recipe ingredients, not a fictitious finished plate.",
  "products.form.restaurantPackagedHint": "Use this for water bottles, cans, packets and other ready-made items you physically count.",
  "products.form.restaurantIngredientHint": "This stays in kitchen inventory and recipe pickers, but never appears to guests.",
  "products.form.recipeTrackedTitle": "Tracked through its recipe",
  "products.form.recipeTrackedHint": "Set the dish recipe in Kitchen stock. Billing will deduct those ingredients and this dish will not create false low-stock or out-of-stock alerts.",
  "products.form.gridStockHint": "Counted from the size grid above.",
  "products.form.reorderLevel": "Reorder Level",
  // The size grid repeats these per row, so they are the short forms — a 72px
  // column cannot carry the full sentence the main form uses.
  "products.form.variantDetails": "Per-size cost, MRP & reorder",
  "products.form.variantMrp": "MRP",
  "products.form.variantCost": "Cost",
  "products.form.variantMinPrice": "Min price",
  "products.form.variantLowStock": "Low stock",
  "products.form.variantReorder": "Reorder at",
  "products.form.variantInheritHint": "Blank cost, MRP or min price follows the product above. Blank low stock or reorder means no alert for this size.",
  "products.form.rateStockUnit": "Rate / Stock Unit",
  "products.form.batchExpiry": "Enable batch and expiry tracking",
  "products.form.batchExpiryTitle": "Batch and expiry tracking",
  "products.form.additionalInfo": "Additional Information",
  "products.form.productImage": "Product Image",
  "products.form.uploadImage": "Upload Image",
  "products.form.replaceImage": "Replace Image",
  "products.form.keepOpen": "Keep panel open to add another product",
  "products.form.save": "Save Product",
  "products.form.update": "Update Product",
  "products.form.nameRequired": "Product name required",
  "products.form.packSizeRequired": "Pack size and price required",
  "products.form.barcodeFilled": "Filled in from the barcode",
  "products.form.packHasStock": "Count this pack to zero first",
  "products.form.packHasStockHint": "It still holds {qty}. Set its stock to 0, save, then remove it — otherwise the goods would be left with nothing counting them.",
  "products.form.packAlreadyAdded": "Pack already added",
  "products.form.chooseDifferentPack": "Choose a different unit or pack size.",
  "products.form.invalidImage": "Invalid image",
  "products.form.imageTooLarge": "Image must be under 2MB.",
  "products.form.imageReadFailed": "Could not read image. Try another file.",
  "products.form.readFailed": "Read failed",
  "products.form.canvasUnsupported": "Canvas unsupported",

  // ── AI alias suggestions ──
  "products.alias.ask": "Ask AI",
  "products.alias.asking": "Asking AI...",
  "products.alias.generate": "Generate name variations",
  "products.alias.suggestions": "Suggestions — tap to add",
  "products.alias.nameFirst": "Type the product name first, then ask AI.",
  "products.alias.noneReturned": "No AI names returned",
  "products.alias.localFallback": "Showing local suggestions instead.",

  // ── Bulk edit ──
  "products.bulk.title": "Bulk edit",
  "products.bulk.applying": "Applying…",
  "products.bulk.ownerPinRequired": "Owner PIN required",
  "products.bulk.ownerPinHint": "Enter the 4-digit owner PIN to change prices in bulk.",
  "products.bulk.savedLocally": "Changes saved locally and will sync.",

  // ── CSV import / migration ──
  "products.import.chooseFile": "Choose a product CSV",
  "products.import.arthaTemplate": "Artha template",
  "products.import.genericCsv": "Generic CSV",
  "products.import.vyaparExport": "Vyapar-style export",
  "products.import.tallyExport": "Tally-compatible export",
  "products.import.readFailed": "Could not read this CSV",
  "products.import.fileTooLarge": "File is too large",
  "products.import.saveAsCsv": "Open the file in Excel and save it as CSV before uploading.",
  "products.import.saving": "Saving migration...",
  "products.import.completed": "Product migration completed",
  "products.import.backupQueued": "Cloud backup has been queued.",
  "products.import.backupWhenOnline": "Cloud backup will continue when this device is online.",
  "products.import.rolledBack": "The migration was rolled back. Review the file and try again.",
  "products.import.nothingPartial": "Nothing was partially imported",
  "products.import.issues": "Issues",
  "products.import.mybillbookExport": "myBillBook-style export",
  "products.import.detectedSource": "Detected {source}. Confirm the financial and packet fields.",
  "products.import.fileTooLargeHint": "Use a CSV smaller than 25 MB. This is enough for well over 10,000 normal product rows.",
  "products.import.importedCounts": "{created} created, {updated} updated. {backup}",
  "products.import.update": "Update",
  "products.import.notSet": "Not set",
  // "Where these are" — the per-branch split under the size grid.
  "products.variantSplit.title": "Where these are",
  "products.variantSplit.size": "Size",
  "products.variantSplit.main": "(main)",
  "products.variantSplit.total": "Total",
  "products.variantSplit.hint": "Counted per size, not in base units. Move stock between branches with a transfer so both sides stay in step.",

  // ── The catalogue in each trade's own words (see settings/shop-products.ts) ──
  // The sidebar already calls this screen "Menu", "Medicines" or "Parts"; these
  // are what the screen itself then has to say back.
  "products.trade.title": "Catalogue for your shop type",
  "products.trade.link.categories": "Categories",
  "products.trade.link.menu": "Menu",

  "products.trade.total.styles": "Total Styles",
  "products.trade.total.models": "Total Models",
  "products.trade.total.parts": "Total Parts",
  "products.trade.total.medicines": "Total Medicines",
  "products.trade.total.titles": "Total Titles",
  "products.trade.total.shades": "Total Shades",
  "products.trade.total.dishes": "Total Dishes",
  "products.trade.total.materials": "Items and Materials",

  "products.trade.hint.styles": "Sizes and colours counted separately",
  "products.trade.hint.models": "Active models",
  "products.trade.hint.parts": "Part numbers on the shelf",
  "products.trade.hint.medicines": "Medicines you stock",
  "products.trade.hint.titles": "Books and supplies",
  "products.trade.hint.shades": "Every shade counted on its own",
  "products.trade.hint.dishes": "Dishes on the menu",
  "products.trade.hint.materials": "Materials, packs and finished goods",

  // What a counter actually types into the box. Naming the wrong identifier
  // tells a shop its catalogue is not searchable that way, when it is.
  "products.trade.search.clothing": "Search by style, size, colour or barcode",
  "products.trade.search.footwear": "Search by model, size or barcode",
  "products.trade.search.autoParts": "Search by part name, OEM number or SKU",
  "products.trade.search.electronics": "Search by model number, brand or barcode",
  "products.trade.search.pharmacy": "Search by medicine, salt or barcode",
  "products.trade.search.stationery": "Search by title, ISBN, class or barcode",
  "products.trade.search.furniture": "Search by model, material or code",
  "products.trade.search.cosmetics": "Search by product, shade or barcode",
  "products.trade.search.restaurant": "Search by dish or menu code",
  "products.trade.search.manufacturing": "Search by material, code or HS code",

  "products.trade.focus.kirana": "One row per pack size. A single row shared by the 500 g and the 1 kg packet will bill one of them at the other's rate.",
  "products.trade.focus.clothing": "A style is not a row: every size and colour is its own row, or availability and returns both stop being exact.",
  "products.trade.focus.footwear": "One row per model and size. A model row holding pooled pairs cannot tell you which size has gone missing.",
  "products.trade.focus.autoParts": "Put the OEM number on the row and the bin beside it — those two fields are most of what makes a parts counter fast.",
  "products.trade.focus.electronics": "Turn on serial tracking for anything that comes back under warranty, then register the pieces on the Serial units screen.",
  "products.trade.focus.pharmacy": "Record the salt, not only the brand. It is what a customer asks for when the brand is out of stock, and what a substitute is found by.",
  "products.trade.focus.stationery": "Books are asked for by class and board far more often than by title — fill those in before you reach for the ISBN.",
  "products.trade.focus.furniture": "Dimensions belong on the row rather than in someone's head: every quotation on the floor is built out of them.",
  "products.trade.focus.cosmetics": "One row per shade, and record the open-jar months — beauty stock expires on the shelf, not in the godown.",
  "products.trade.focus.restaurant": "The menu is the catalogue. Keep the allergen and onion-garlic answers on the dish so a server never has to walk back to the kitchen.",
  "products.trade.focus.manufacturing": "Classify every item as raw, packaging or finished good — that is what decides how it is counted, costed and traced.",
  "products.trade.focus.other": "Give every sellable item a unique code, its real selling unit and a reorder level; most of the rest of the app reads those three.",

  // Voice dictation in the add/edit form. The prompts are questions a shopkeeper
  // would actually be asked across a counter, not field labels read aloud.
  "products.voice.start": "Fill by voice",
  "products.voice.listening": "Listening...",
  "products.voice.starting": "Starting the mic...",
  "products.voice.stop": "Stop",
  "products.voice.speakToggle": "Read the questions aloud",
  "products.voice.heard": "Heard: {text}",
  "products.voice.ready": "Everything needed is filled. Check it and press Save.",
  "products.voice.notUnderstood": "Did not catch that. Say it again, or say skip.",
  "products.voice.unsupported": "This browser cannot listen. Type the details instead.",
  "products.voice.hint": "Say it all at once, like Tata Salt 1 kg MRP 28 cost 24 selling 26, or answer one question at a time.",
  "products.voice.controls": "Say skip to pass a question, save when you are done.",
  "products.voice.filled": "{count} details filled by voice.",
  "products.voice.ask.name": "What is the product called?",
  "products.voice.ask.sellingPrice": "Selling price?",
  "products.voice.ask.mrp": "MRP printed on the pack?",
  "products.voice.ask.costPrice": "What does it cost you?",
  "products.voice.ask.stockQuantity": "How much stock is coming in?",
  "products.voice.ask.category": "Which category?",
  "products.voice.ask.brand": "Which brand?",
  "products.voice.ask.unit": "Sold by which unit?",
  "products.voice.ask.packSize": "What does one pack hold?",
  "products.voice.ask.barcode": "Barcode number?",
  "products.voice.ask.hsn": "HSN code?",
  "products.voice.ask.aliases": "Any other names for it?",
  "products.voice.ask.gstRate": "GST rate?",
  "products.voice.ask.minimumSellingPrice": "Minimum selling price?",
  "products.voice.ask.retailPrice": "Retail price?",
  "products.voice.ask.wholesalePrice": "Wholesale price?",
  "products.voice.ask.lowStockAlert": "Alert when stock falls to?",
} as const;
