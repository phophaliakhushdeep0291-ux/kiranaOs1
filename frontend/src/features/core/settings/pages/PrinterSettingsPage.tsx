          ...
          <Card>
            ...
            <div className="space-y-3 px-5 pb-5">
              ...
              <div className="grid grid-cols-1 gap-2 pt-1 sm:grid-cols-2">
                ...
                <Button variant="outline" className="h-11 gap-1.5 rounded-[9px] text-[12px] font-bold sm:mouse:h-9" onClick={() => void connectPrinter()} disabled={connecting}><Cable size={14} /> {connecting ? t("settings.printer.connecting") : t("settings.printer.connect")}</Button>
          ...
          <div className="px-5 pb-4">
            {jobs.length === 0 ? (
              ...
              <div key={j.id} className={`flex items-center gap-3 py-2.5 ${i < jobs.length - 1 ? "border-b border-[#eef2f8]" : ""}`}>
                ...
                {j.status === "failed" ? <Button size="sm" variant="outline" className="h-11 gap-1 rounded-[8px] text-[12px] font-bold sm:mouse:h-8" onClick={testPrint}><RefreshCcw size={12} /> {t("settings.printer.retry")}</Button> : <Badge tone={j.status === "pending" ? "blue" : "green"}>{j.status === "saved" ? t("settings.printer.jobSaved") : j.status === "sent" ? t("settings.printer.jobSent") : j.status === "opened" ? t("settings.printer.jobOpened") : t("settings.printer.jobSending")}</Badge>}
              </div>
            ))}
      ...
      <Card>
        ...
        <div className="grid gap-3 px-5 pb-5 sm:grid-cols-2 xl:grid-cols-6">
          {hardwareCapabilities.map((capability) => {
            ...
            return <div key={capability.label} className="rounded-xl border border-[#e5eaf2] bg-[#f8fafc] p-3"><div className="flex items-center justify-between"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white text-[var(--brand)] shadow-sm"><Icon size={16} /></span><Badge tone={capability.ready ? "green" : "amber"}>{capability.ready ? t("settings.printer.capabilityAvailable") : t("settings.security.unavailable")}</Badge></div><p className="mt-3 text-[12px] font-black text-[var(--brand-ink)]">{capability.label}</p><p className="mt-1 text-[10.5px] leading-4 text-[#64748b]">{capability.detail}</p></div>;
          })}
        </div>
      </Card>
    </SettingsShell>
