  ...
  return (
    <div className="mt-1">
      <button
        ...
        aria-expanded={open}
        aria-label={`Choose batch for ${productName}`}
        className={cn(
      ...
      >
        ...
        {selected ? `Batch ${selected.batchNumber}` : "Nearest expiry"}
      </button>
      ...
      {open ? (
        <div className="mt-1 max-h-44 w-full max-w-[260px] overflow-y-auto rounded-md border bg-white p-1 shadow-sm">
          ...
          >
            Nearest expiry (automatic)
          </button>

          {query.isLoading ? (
            <p className="px-2 py-2 text-[10px] text-[#9aa7bd]">Loading batches…</p>
          ) : batches.length === 0 ? (
            <p className="px-2 py-2 text-[10px] text-[#9aa7bd]">No saleable batches at this branch.</p>
          ) : (
            batches.map((batch) => {
