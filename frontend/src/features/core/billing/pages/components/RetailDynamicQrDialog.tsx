...
import { checkHardwareBridge, printQrSlipViaHardwareBridge } from "@/features/core/hardware/local-hardware-bridge";
...
export function RetailDynamicQrDialog({ checkout, onConfirmed, onClose, onStatusChange }: RetailDynamicQrDialogProps) {
  ...
  // paired local bridge, and only bridges new enough to own the raster path can
  // print a QR at all. Offering the action on an older bridge just produces a
  // confusing "endpoint not found" at the moment a customer wants to pay.
  const [printSupport, setPrintSupport] = useState<"unavailable" | "outdated" | "ready">("unavailable");
  const expiresAt = checkout ? new Date(checkout.expiresAt).getTime() : 0;
  const remaining = Number.isFinite(expiresAt) ? Math.max(0, expiresAt - now) : 0;
  const waiting = ["creating", "pending"].includes(status);
  const collectable = waiting && remaining > 0;
  ...
  // Probe the paired bridge when the QR opens rather than assuming: a counter
  // still on an older bridge reports no qrPrint capability, and a bridge that is
  // simply down should not advertise an action that cannot run.
  useEffect(() => {
    if (!checkout) return;
    const printer = getPrinterConfigSync();
    if (printer.connection !== "bridge") { setPrintSupport("unavailable"); return; }
    let active = true;
    void checkHardwareBridge(printer.bridgeUrl)
      .then((health) => { if (active) setPrintSupport(health.capabilities?.qrPrint === true ? "ready" : "outdated"); })
      .catch(() => { if (active) setPrintSupport("unavailable"); });
    return () => { active = false; };
  }, [checkout]);
  ...
  async function printSlip() {
    if (!checkout || printing || !collectable) return;
    setPrinting(true);
    setError(null);
    try {
      const printer = getPrinterConfigSync();
  ...
  return (
    <Dialog open={Boolean(checkout)} onOpenChange={(open) => { if (!open) { if (terminal || status === "confirmed") onClose(); else void cancel(); } }}>
      <DialogContent className="max-w-md" onEscapeKeyDown={(event) => { if (cancelling) event.preventDefault(); }}>
        ...
        {checkout ? <div className="space-y-4">
          ...
          {collectable && trustedImage ? <div className="grid place-items-center rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><img src={trustedImage} alt={t("billing.pay.dynamicQr.imageAlt", { amount: (checkout.amountPaise / 100).toFixed(2) })} className="h-56 w-56 max-w-full rounded-xl object-contain" referrerPolicy="no-referrer" /></div> : null}

          {waiting ? <div role="status" className="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-blue-800"><Loader2 className="mt-0.5 animate-spin" size={16} /><div><p className="font-black">{t("billing.pay.dynamicQr.waiting")}</p><p>{t("billing.pay.dynamicQr.waitingHelp")}</p>{!collectable ? <p className="mt-1 font-semibold">{t("billing.pay.dynamicQr.expiredChecking")}</p> : null}</div></div> : null}
          {status === "confirmed" ? <div role="status" className="flex items-center gap-2 rounded-xl bg-emerald-50 p-3 text-sm font-black text-emerald-800"><CheckCircle2 size={18} /> {t("billing.pay.dynamicQr.confirmed")}</div> : null}
          ...
          {printSupport === "outdated" && collectable ? <p className="text-[11px] font-semibold text-slate-500">{t("billing.pay.dynamicQr.printNeedsUpdate")}</p> : null}
        ...
        <DialogFooter>
          {terminal || status === "confirmed" ? <Button onClick={onClose}>{t("billing.pay.dynamicQr.close")}</Button> : <>
            {printSupport === "ready" ? <Button variant="outline" className="gap-1.5" disabled={printing || cancelling || !collectable} onClick={() => void printSlip()}>
