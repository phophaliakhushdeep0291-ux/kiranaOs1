...
export function BillingPaymentPanel({
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-[12px] font-extrabold text-[#3D4354]">{t("billing.pay.method")}</p>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {tenderSuggestions.map((amount) => (
                <button
                  ...
                  className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-[#dbe3ef] bg-white px-2.5 py-1 text-[12px] font-bold text-[#585E76] transition-colors hover:border-[var(--brand)] hover:text-[var(--brand)]"
                >
                  {fmtRs(amount)}
                </button>
              ))}
