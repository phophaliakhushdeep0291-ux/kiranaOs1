...
function buildStockProjection(items: BillInputItem[], productsById: Map<string, Product>) {
  ...
  for (const item of items) {
    ...
    if (!product) continue;
    // A prepared dish is a menu/price identity. The server consumes its recipe
    // ingredients; decrementing the plate itself in the offline projection is
    // what created the negative dish rows seen on the restaurant inventory page.
