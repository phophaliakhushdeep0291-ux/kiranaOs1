...
let cache: PaymentConfig = { upiId: "", payeeName: "Veyra merchant" };
...
export function setPaymentConfigCache(bank: Record<string, unknown> | undefined, shopName?: string | null) {
  cache = {
    ...
    payeeName: typeof bank?.holder === "string" && bank.holder.trim() ? bank.holder.trim() : (shopName?.trim() || "Veyra merchant"),
  };
}
...
export function buildUpiPaymentUri({ upiId, payeeName, amount, note }: PaymentConfig & { amount: number; note?: string }) {
  ...
  const query = new URLSearchParams({ pa: upiId, pn: payeeName || "Merchant", am: amount.toFixed(2), cu: "INR", tn: note || "Veyra counter payment" });
