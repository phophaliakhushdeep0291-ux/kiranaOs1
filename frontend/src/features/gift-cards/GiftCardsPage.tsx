...
export default function GiftCardsPage() {
  ...
  return <div className="space-y-5 pb-10">
    ...
    <Dialog open={Boolean(issued)} onOpenChange={(open) => { if (!open) setIssued(null); }}><DialogContent className="max-w-md"><DialogHeader><DialogTitle className="flex items-center gap-2"><Check className="text-emerald-600" /> Gift card ready</DialogTitle><DialogDescription>Share this code with the customer now. For security, Veyra will show only the last four characters after closing.</DialogDescription></DialogHeader><div className="rounded-2xl bg-gradient-to-br from-pink-600 to-violet-700 p-5 text-white"><p className="text-xs font-bold uppercase tracking-[0.2em] text-pink-100">Veyra Gift Card</p><p className="mt-5 select-all break-all font-mono text-xl font-black tracking-wider">{issued?.code}</p><div className="mt-5 flex items-end justify-between"><div><p className="text-[10px] uppercase text-pink-100">Available</p><p className="text-2xl font-black">{money(issued?.balance ?? 0)}</p></div><Gift size={30} className="text-pink-100" /></div></div><DialogFooter><Button variant="outline" onClick={() => void copyCode()}>{copied ? <Check size={15} /> : <Copy size={15} />} {copied ? "Copied" : "Copy code"}</Button><Button onClick={() => setIssued(null)}>I have shared it</Button></DialogFooter></DialogContent></Dialog>
