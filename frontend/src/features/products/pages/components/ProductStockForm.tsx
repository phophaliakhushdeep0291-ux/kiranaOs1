  ...
  return (
    <>
      ...
      <div className="flex items-start justify-between gap-4 rounded-xl border bg-muted/40 p-3">
        <div><p className="font-medium text-sm">Batch and expiry tracking</p><p className="text-xs text-muted-foreground">Require a batch number and expiry when this product is received. Checkout automatically uses the earliest-expiring saleable stock.</p>{!batchFeature.loading && !batchFeature.allowed && <p className="mt-1 text-xs font-semibold text-amber-700">Available on {batchFeature.requiredPlan.name}.</p>}</div>
        <Switch checked={batchTracking} disabled={batchFeature.loading || !batchFeature.allowed} onCheckedChange={(checked) => form.setValue("batchTrackingEnabled", checked, { shouldDirty: true })} aria-label="Enable batch and expiry tracking" />
      </div>
    </>
  );
}
