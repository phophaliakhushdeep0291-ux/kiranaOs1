...
const EXECUTORS: Record<SupportCommandType, CommandExecutor> = {
  ...
  async RUN_SYNC_NOW() {
    const { runSyncCycle } = await import("@/features/sync/sync-engine");
  ...
  async RETRY_FAILED_SYNC() {
    const { retryFailedSyncOperations } = await import("@/features/sync/sync-engine");
    const result = await retryFailedSyncOperations();
    return { ...result };
  },

  async PULL_FROM_CLOUD() {
    const { hydrateFromBackendSnapshot } = await import("@/features/sync/cloud-hydration");
