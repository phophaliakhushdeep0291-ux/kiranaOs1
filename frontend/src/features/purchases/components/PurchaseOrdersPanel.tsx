  ...
  return (
    <div className="mt-2 rounded-lg border border-blue-100 bg-blue-50/60 px-2.5 py-2 text-[10.5px] leading-relaxed text-[#52627e]">
      <div className="mb-1 flex flex-wrap items-center gap-1.5">
        <span className="font-black text-[#075fff]">{CONFIDENCE_LABEL[suggestion.forecastConfidence]}</span>
  ...
  return (
    <section className="rounded-2xl border border-[#dfe7f3] bg-[#fbfdff] p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div><p className="text-sm font-black text-[#102347]">PO · Goods receipt · Supplier invoice</p><p className="mt-0.5 text-[11px] text-[#64748b]">Every receipt remains pending until its supplier invoice evidence is matched or an owner approves the documented variance.</p></div>
        <span className={cn("rounded-full px-2.5 py-1 text-[10px] font-black", summaryTone.cls)}>{summaryTone.label}</span>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        <div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-[9px] font-bold uppercase text-[#71809b]">PO value received</p><p className="mt-1 font-black text-[#102347]">{inr(order.reconciliation.expectedGoodsAmount)}</p><p className="text-[10px] text-[#64748b]">Rate variance {inr(order.reconciliation.priceVarianceAmount)}</p></div>
        <div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-[9px] font-bold uppercase text-[#71809b]">GRN value</p><p className="mt-1 font-black text-[#102347]">{inr(order.reconciliation.goodsReceivedAmount)}</p><p className="text-[10px] text-[#64748b]">{order.reconciliation.receiptCount} receipt{order.reconciliation.receiptCount === 1 ? "" : "s"}</p></div>
        <div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-[9px] font-bold uppercase text-[#71809b]">Invoice evidence</p><p className="mt-1 font-black text-[#102347]">{inr(order.reconciliation.supplierInvoiceAmount)}</p><p className="text-[10px] text-[#64748b]">{order.reconciliation.invoicePendingCount} pending</p></div>
      </div>
      {order.receipts.length > 0 && <div className="mt-3 space-y-2">{order.receipts.map((receipt) => { const tone = RECONCILIATION_STATUS[receipt.matchStatus] ?? RECONCILIATION_STATUS.invoice_pending; return <article key={receipt.id} className="rounded-xl border border-[#e5ebf4] bg-white p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black text-[#102347]">{receipt.receiptNumber}</p><p className="mt-0.5 text-[10px] text-[#64748b]">{receipt.supplierInvoiceNumber || "No supplier invoice attached"} · {new Date(receipt.createdAt).toLocaleDateString("en-IN")}</p></div><span className={cn("rounded-full px-2 py-0.5 text-[9px] font-black", tone.cls)}>{tone.label}</span></div><div className="mt-2 grid grid-cols-3 gap-2 text-[10px]"><div><span className="text-[#71809b]">PO</span><p className="font-bold text-[#314566]">{inr(receipt.expectedGoodsAmount)}</p></div><div><span className="text-[#71809b]">GRN</span><p className="font-bold text-[#314566]">{inr(receipt.totalAmount)}</p></div><div><span className="text-[#71809b]">Invoice</span><p className="font-bold text-[#314566]">{receipt.supplierInvoiceAmount == null ? "Pending" : inr(receipt.supplierInvoiceAmount)}</p></div></div>{receipt.varianceReason && <p className="mt-2 rounded-lg bg-violet-50 px-2.5 py-2 text-[10px] text-violet-800"><strong>Approved reason:</strong> {receipt.varianceReason}</p>}{receipt.matchStatus === "invoice_pending" && <Button type="button" variant="outline" size="sm" className="mt-2 h-8 text-xs" onClick={() => onReconcile(order, receipt)}>Reconcile invoice</Button>}</article>; })}</div>}
    </section>
  );
}
...
export function PurchaseOrdersPanel() {
  ...
  const reviewReconciliation = () => {
    ...
    setApproval({
      ...
      run: async (pin) => {
        const order = await reconcilePurchaseReceipt(target.order.id, target.receipt.id, { supplierInvoiceNumber: reconcileInvoiceNumber.trim(), supplierInvoiceAmount: invoiceAmount, varianceReason: hasVariance ? reconcileVarianceReason.trim() : undefined }, pin);
        await refresh();
        setViewOrder(order);
