...
import { canCancelTradeOrder, hasOutstandingTradeQuantity, nextDispatchNumber, lineCountKey, localDay, tradeOrderDocuments, tradeOrderStatusKey } from "../trade-order-status";
...
type TradeOrder = {
  ...
  billId?: string | null; customerId?: string | null; dispatches?: Array<{ id: string; dispatchNumber: string; billId?: string | null }>;
...
const tradeActionKey = {
  ...
  partially_dispatched: "manufacturing.orders.action.confirmed",
    ...
    <PageShell className="space-y-4 px-3 py-3 sm:px-4 sm:py-4 lg:space-y-5 lg:px-6 lg:py-5" data-testid="manufacturing-page">
      ...
      <section id="trade-orders" className="scroll-mt-20 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className={panel}>
          ...
          {dispatchOrderId ? <div className="grid gap-2 border-b border-amber-200 bg-amber-50 p-4 sm:grid-cols-2"><Input aria-label={t("manufacturing.orders.dispatchNumber")} placeholder={t("manufacturing.orders.dispatchNumber")} value={dispatchNumber} onChange={(event) => setDispatchNumber(event.target.value)} /><Input aria-label={t("manufacturing.orders.transporter")} placeholder={t("manufacturing.orders.transporter")} value={transporterName} onChange={(event) => setTransporterName(event.target.value)} /><Input aria-label={t("manufacturing.orders.vehicle")} placeholder={t("manufacturing.orders.vehicle")} value={vehicleNumber} onChange={(event) => setVehicleNumber(event.target.value)} /><Input aria-label={t("manufacturing.orders.awb")} placeholder={t("manufacturing.orders.awb")} value={lrAwbNumber} onChange={(event) => setLrAwbNumber(event.target.value)} /><Input aria-label={t("manufacturing.orders.eway")} placeholder={t("manufacturing.orders.eway")} value={ewayBillNumber} onChange={(event) => setEwayBillNumber(event.target.value)} /><Input aria-label={t("manufacturing.orders.shippingBill")} placeholder={t("manufacturing.orders.shippingBill")} value={shippingBillNumber} onChange={(event) => setShippingBillNumber(event.target.value)} />{dispatchOrder?.orderType === "export" ? <p role="alert" className="text-xs leading-5 text-amber-900 sm:col-span-2">{t("manufacturing.orders.exportDispatchWarning")}</p> : null}<Button disabled={!dispatchNumber.trim() || dispatchTradeOrder.isPending} onClick={() => dispatchTradeOrder.mutate()}>{t("manufacturing.orders.action.packed")}</Button><Button variant="outline" onClick={() => setDispatchOrderId("")}>{t("manufacturing.orders.cancelDispatch")}</Button></div> : null}
            ...
            {(tradeOrdersQ.data ?? []).map((order) => {
              ...
              return <div key={order.id} className="grid gap-3 p-4 text-sm sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:col-span-3">
                  {order.status in tradeActionKey && (order.status !== "partially_dispatched" || hasOutstandingTradeQuantity(order)) ? <Button size="sm" className="min-h-11" disabled={advanceTradeOrder.isPending} onClick={() => advanceTradeOrder.mutate(order)}>{t(tradeActionKey[order.status as keyof typeof tradeActionKey])}</Button> : null}
                  {order.status === "packed" ? <Button size="sm" className="min-h-11" onClick={() => { setDispatchOrderId(order.id); setDispatchNumber(nextDispatchNumber(order)); }}>{t("manufacturing.orders.action.packed")}</Button> : null}
                  {["dispatched", "partially_dispatched"].includes(order.status) && order.dispatches?.some((dispatch) => !dispatch.billId) ? <Button size="sm" className="min-h-11" onClick={() => setInvoiceOrderId(order.id)}>{t("manufacturing.invoice.create")}</Button> : null}
