...
interface Program { active: boolean; pointsPerRupee: number; redemptionPaisePerPoint: number; minimumRedeemPoints: number; configured?: boolean }
interface Account { id: string; pointsBalance: number; lifetimeEarned: number; lifetimeRedeemed: number; customer: { id: string; name: string; mobile?: string | null } }
...
export default function LoyaltyPage() {
  ...
  const accountsQ = useQuery({ queryKey: ["loyalty-accounts"], queryFn: () => apiRequest<Account[]>("/loyalty/accounts?limit=200") });
  ...
  const save = useMutation({
    mutationFn: () => apiRequest<Program>("/loyalty/program", { method: "PUT", ownerPin, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active, pointsPerRupee: Number(earnRate), redemptionPaisePerPoint: Number(pointValue), minimumRedeemPoints: Number(minimum), ownerPin }) }),
