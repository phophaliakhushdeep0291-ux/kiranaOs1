    ...
    <PageShell className="space-y-4 bg-white p-4 sm:p-5 2xl:p-6">
      <section className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between 2xl:fixed 2xl:right-[276px] 2xl:top-[18px] 2xl:z-[60] 2xl:w-auto 2xl:flex-row 2xl:gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button onClick={() => tab === "purchase" ? navigate("/purchase-bills") : setBuilderOpen(true)} className="h-10 gap-2 rounded-[8px] bg-[var(--brand)] px-5 text-[12px] font-bold text-white shadow-[0_8px_18px_rgba(7,95,255,0.2)] hover:bg-[#0052e0]"><Plus size={15} />{tab === "purchase" ? "Supplier Return" : "New Return"}</Button>
        </div>
      </section>

      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        <MetricCard label="Total Returns" value={inr(metrics.total)} previous={previousMetrics.total} current={metrics.total} color="#1768f5" icon={<ShoppingBag size={16} />} iconClass="bg-[var(--brand-soft)] text-[#1768f5]" spark={spark.map((row) => row.total)} />
        ...
        <MetricCard label="Credit Issued (Udhar)" value={inr(metrics.credit)} previous={previousMetrics.credit} current={metrics.credit} color="#1768f5" icon={<CreditCard size={16} />} iconClass="bg-[var(--brand-soft)] text-[#1768f5]" spark={spark.map((row) => row.credit)} />
      </section>
