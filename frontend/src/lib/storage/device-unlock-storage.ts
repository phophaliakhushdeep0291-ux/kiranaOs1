...
// Salted PBKDF2 verifier for the owner PIN, written only after the server has
// accepted that PIN. Same rules: device-local, never synced, never backed up.
export const COUNTER_PIN_OFFLINE_KEY = "kiranaos.security.offlineCounterPin.v1";

export function clearDeviceUnlock(): void {
  if (typeof window === "undefined") return;
  for (const key of [DEVICE_UNLOCK_KEY, LEGACY_DEVICE_UNLOCK_KEY, COUNTER_PIN_OFFLINE_KEY]) {
