...
| Manufacturing | Multi-material recipes, split source batches, mixed output packs, production planning/completion, saved production drafts and QC release are available. A recovered mobile draft consumed two tracked source batches and produced 20 units in two pack sizes with exact stock and ledger balances. Production through allocation/packing/dispatch is covered by transactional tests. | Wholesale invoice posting, return accounting and export documents still need a combined end-to-end pass. Production drafts are local to the device; recipe and wholesale-order drafts do not yet share this recovery feature. |
...
1. Complete wholesale invoice posting and returns as one reconciled accounting flow. Split source batches, mixed output packs, saved production entry and guarded allocation/dispatch are implemented and verified below.
...
The accounting gap remains explicit: the existing invoice-link action links an already-created bill; it does not itself create a reconciled financial invoice from dispatched stock. Do not treat that action or a generated trade PDF as proof of a complete production-to-invoice/credit-note flow. That work, rental/furniture money reconciliation, electronics billing/repair integration, durable AI handoffs and the external release checks remain on the readiness list.
