...
| ADM-001 | Roles, owner PIN, device limits and tenant isolation are enforced server-side. | RBAC, session and isolation tests. | Verified locally. Staff create/edit/deactivate requires online server confirmation; cached offline staff is read-only. Staff mutations and their required owner audit share one transaction, password changes revoke active sessions, concurrent Growth-plan invitations admit exactly five seats while rejecting the sixth under SQLite and PostgreSQL-safe locks, and a disabled historical user releases one replacement seat. Isolated owner/RBAC integration passes 17/17; candidate CI evidence remains required. | P0 |
