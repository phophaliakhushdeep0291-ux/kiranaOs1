...
| BILL-008 | Owner approval is enforced for configured discount thresholds and sensitive edits. | Permission/audit tests; cashier-versus-owner QA. | Locally fixed. Online and offline bill confirmation now derive sensitive actions from authoritative product, selling-unit and pricing-rule data; client-declared actions and forged/stale rule IDs cannot bypass the owner PIN. The same transaction records owner, reason and device audit evidence, while any commercial cart edit invalidates the approval fingerprint. Focused frontend approval coverage passes 8/8, the authoritative server-rule regression passes 1/1, the complete backend harness passes, the full frontend suite passes 1,426 with one intentional skip, and production/typecheck/migration/release gates pass. Live cashier-versus-owner phone evidence remains pending because the browser-control runtime cannot start under the current Windows sandbox. | P0 |
