...
| ADM-002 | Settings expose billing, tax, printer, backup, security and sync safely on mobile. | Permission tests; settings mobile QA. | Partially verified. Subscription/Staff has server-authoritative transition and four-width layout evidence in `docs/evidence/mobile-staff/`. The Settings landing, billing, tax, printer, sync/backup and security pages now pass 24/24 live captures at 375x667, 390x844, 430x932 and 768x1024 with exact document/viewport width parity, zero horizontal overflow and zero active controls below 44px. Stateful 390px checks cover subscription refresh, reversible tax/security controls, printer save feedback, Sync Now, owner-PIN backup creation and Change PIN cancellation. An isolated keyed backend created, decrypted/previewed and protected-downloaded a 175-record AES-256-GCM artifact, then the artifact, object and QA sessions were removed. The primary runtime correctly surfaced its missing `BACKUP_ENCRYPTION_KEY`; exact-runtime UI creation/restore plus physical printer and external delivery certification remain open. Evidence: `docs/evidence/mobile-settings/manifest.json`. | P1 |
