...
The permission that matters is **loopback**, not "local network". Chrome keeps them apart: granting the `loopback_network` content setting for the frontend origin makes the same request succeed in ~10ms, while granting `local_network` leaves it hanging exactly as before — and does so while `navigator.permissions.query({ name: "local-network-access" })` reports `"granted"`. That query is therefore useless as a readiness check for the bridge; only an actual request settles the question.
